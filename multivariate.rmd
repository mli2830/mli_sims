...
```{r MCMCglmm fit, warning=FALSE,message=FALSE,eval=FALSE}
...
```{r MCMCglmm results,warning=FALSE,meggage=FALSE}

load(".simulate.mcmc.RData")

summary(mfit)

```

The mixing is very bad. (see eff.samp). If the results look correct than it is fine. We can increase the MCMC chains more and see if the results looks correct for the prince of computation time. Another way to test is to model in the univariate random effect framework and see if the results are sensible.

```{r univariate df, message=FALSE,warning=FALSE}

load(".simulate.lme4.RData")
...
The results from univariate/ glm fit is not good too. The reason is because lme4 is using laplace approximation and random effect sample size needs to be large. We are in the worst possible case scenario of having only size 2 (couple size). The way around this is to use adaptive/Hermite gaussian quadrature. 
